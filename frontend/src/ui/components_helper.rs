...
use eframe::Theme;
use egui::{Direction, FontSelection, Image, Layout, Response, RichText, TextBuffer, TextFormat, TextStyle};
...
use shared::fiche_rp::{FicheRP, FicheState, Job, ScienceLevel, ScienceRole, SecurityLevel, SecurityRole};
...
pub fn ficherp_viewer(ui: &mut egui::Ui, ficherp: &FicheRP, user: &User, cache: Arc<RwLock<CommonMarkCache>>) {
    ...
    ui.vertical(|ui| {
        ...
        egui::ScrollArea::vertical().id_source("scoll_text_viewer").show(ui, |ui| {
            ui.label(RichText::new("Description physique : ").text_style(TextStyle::Name("heading3".into())));

            CommonMarkViewer::new("desc_viewer").show(ui, &mut cache, &ficherp.description);
            ui.separator();

            ui.label(RichText::new("Lore : ").text_style(TextStyle::Name("heading3".into())));
...
pub fn ficherp_edit(ui: &mut egui::Ui, ficherp: &mut FicheRP, cache: Arc<RwLock<CommonMarkCache>>, is_previewing: &mut bool) {
    ...
    ui.vertical(|ui| {
        ...
        ui.horizontal(|ui| {
            ...
            ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                if ui.button("Preview de la fiche").clicked() {
                    *is_previewing = true;
                }
            });
        });
